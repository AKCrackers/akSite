...
- Customer list
- Payment list
- MongoDB persistence for the live application store
...
- `server/db.js`: MongoDB persistence, catalogue seeding, and admin account reconciliation.
...
- MongoDB stores the application state in the `store` collection as one document with `_id: main`.
...
- `NODE_ENV=production`
- `MONGODB_URI` with the live MongoDB connection string
- `JWT_SECRET` with a new random value of at least 32 characters
- `ADMIN_EMAIL` and a strong `ADMIN_PASSWORD`
- `DEMO_PAYMENT=false`
- `FRONTEND_ORIGINS` to the live frontend origin when frontend and API use different domains

The server stores product images under `server/uploads`. Render's normal filesystem is ephemeral, so configure a persistent disk mounted at that path or move uploads to object storage before relying on uploaded photos in production. Keep the service at one instance unless persistence is moved to a shared storage design; the application state is cached in memory and persisted as one MongoDB document.

Use HTTPS, backups, audit logging, and Razorpay webhooks for a hardened payment deployment.

## Product visibility / old-data self-heal
On server startup, the app reconciles the MongoDB store document against the supplied 125-product 2026 catalogue. This fixes older installations that had only a small starter catalogue while preserving matching stock and uploaded photos.
